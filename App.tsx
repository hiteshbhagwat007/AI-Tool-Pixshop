...
type Tab = 'retouch' | 'faces' | 'adjust' | 'filters' | 'crop';
...
const App: React.FC = () => {
  ...
  const [detectedFaces, setDetectedFaces] = useState<Detection[]>([]);
  ...
  const handleImageLoad = useCallback(async (e: React.SyntheticEvent<HTMLImageElement>) => {
    ...
    try {
        const faces = await detectFaces(e.currentTarget);
  ...
  const handleSelectFace = (index: number) => {
    if (!imgRef.current || !detectedFaces[index]) return;

    const face = detectedFaces[index];
    const { boundingBox } = face;

    // Bounding box coords are in natural image dimensions
    const centerX = boundingBox.originX + boundingBox.width / 2;
    const centerY = boundingBox.originY + boundingBox.height / 2;
    
    setEditHotspot({ x: Math.round(centerX), y: Math.round(centerY) });
    
    // Calculate display hotspot based on current rendered image size
    const { naturalWidth, naturalHeight, clientWidth, clientHeight } = imgRef.current;
    const scaleX = clientWidth / naturalWidth;
    const scaleY = clientHeight / naturalHeight;

    setDisplayHotspot({ x: centerX * scaleX, y: centerY * scaleY });
  ...
  const renderContent = () => {
    ...
    const imageDisplay = (
      <div className="relative">
        ...
        />
        {activeTab === 'faces' && imgRef.current && detectedFaces.length > 0 && (
            <div className="absolute top-0 left-0 w-full h-full pointer-events-none">
                {detectedFaces.map((face, index) => {
                    ...
                    const style = {
                        left: `${face.boundingBox.originX * scaleX}px`,
                        top: `${face.boundingBox.originY * scaleY}px`,
                    ...
                    return (
                        <div
                            key={index}
                            onClick={(e) => { e.stopPropagation(); handleSelectFace(index); }}
                            className={`absolute border-2 rounded-md transition-all duration-200 cursor-pointer pointer-events-auto hover:border-blue-300 hover:bg-blue-500/10 ${
                                selectedFaceIndex === index ? 'border-blue-400 ring-2 ring-blue-400 ring-offset-2 ring-offset-black/50' : 'border-white/70'
                            }`}
                            style={style}
                        />
                    );
    ...
    return (
      <div className="w-full max-w-4xl mx-auto flex flex-col items-center gap-6 animate-fade-in">
        ...
        <div className="w-full bg-gray-800/80 border border-gray-700/80 rounded-lg p-2 flex items-center justify-center gap-2 backdrop-blur-sm">
            {(['retouch', 'faces', 'crop', 'adjust', 'filters'] as Tab[]).map(tab => (
                 <button
                    key={tab}
        ...
        <div className="w-full">
            ...
            {activeTab === 'faces' && <FacePanel faces={detectedFaces} onSelectFace={handleSelectFace} isLoading={isDetectingFaces} />}
